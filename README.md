...
- ✅ **差分サマリーシート作成選択**（必要に応じて作成可能、デフォルトは作成しない）
- ✅ **HTMLレポート自動生成**（処理結果を視覚的に確認できる統計レポート）
- ✅ **6色のハイライト色を選択可能**（青・緑・オレンジ・紫・ピンク・赤）
- ✅ **比較モード選択**（表示値のみ / 数式も含む）
- ✅ **数式・日付書式の完全保持**（数式セルや日付セルの書式が維持される）
...
- **差分サマリーシート**（オプション）: 作成を選択した場合、最初のシートに変更箇所の一覧が作成されます
  - No.（通し番号）
  - シート名（変更があったシート）
  - セル（セル番地：A1, B2など）
  - 旧値（変更前の値、100文字まで）
...
- **ハイライト範囲**: 
  - 通常のテキストセル: 差分がある文字のみハイライト（文字単位）
  - 数式セル・日付セル: セル全体をハイライト（書式と数式を完全保持）
- **フォーマット維持**: フォントサイズ、フォント名、下線などは元のまま保持
- **数式・書式の保持**: 数式（`=MAX(...)`など）や日付書式（`2025/11`など）が完全に維持される
...
5. **検索・フィルター機能**
   ...
   - 差分内容で検索（旧値・新値を横断検索）
   - **複数選択フィルター**（Excelライクな操作）
     - シートフィルター: 複数のシートを選択して絞り込み
     - ステータスフィルター: 成功/エラーを選択
     - 差分種別フィルター: 追加/削除/変更を組み合わせて表示
   - **「すべて選択」チェックボックス**: 各フィルターグループで一括選択/解除が可能
   - 中間状態表示: 一部だけ選択されている状態を視覚的に表現
   - リアルタイム反映: チェックボックスをクリックした瞬間にフィルタリング
...
4. ソート機能で見たい順序に並び替え
5. 検索ボックスで特定のファイルや差分内容を素早く検索
6. フィルター機能で条件を絞り込み
   - シート、ステータス、差分種別を複数選択可能
   - 「すべて選択」で一括チェック/解除
   - リアルタイムで結果が反映されます
7. グラフで差分の多いファイルを視覚的に把握
8. リセットボタンで全てのフィルターを初期状態に戻す
...
- **数式セルや日付セルの書式保持**: 数式や特殊な書式を持つセルは、セル全体がハイライトされます（文字単位ではなく、書式・数式を完全保持するため）
...
### 日付や数値の表示形式が変わる場合
- 最新版では数式セルや日付セルの書式が自動的に保持されます
- `=MAX(...)`のような数式や`2025/11`のような日付書式は維持されます
- これらのセルはセル全体がハイライトされます（書式保持のため）
...
### 差分サマリーシートが表示されない場合
- **デフォルトでは作成されません**: 起動時に「作成する」を選択した場合のみ作成されます
- 差分が1件も検出されなかった場合も、サマリーシートは作成されません
